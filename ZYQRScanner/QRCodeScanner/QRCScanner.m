...
- (void)updateLayout{
   ...
   // CGSize screenSize = screenRect.size;
    ...
    //中间清空的矩形框
    _clearDrawRect = CGRectMake(kWidth/ 2 - transparentArea.width / 2,
                                      kHeight / 2 - transparentArea.height / 2,
...
- (void)addNoticeInfoLable:(CGRect)rect{
    _noticeInfoLable = [[UILabel alloc]initWithFrame:CGRectMake(0, (rect.origin.y + rect.size.height+10),kWidth, 20)];
    [_noticeInfoLable setText:@"将书籍条形码放入取景框中即可自动扫描"];
...
- (void)addLightButton:(CGRect)rect{
    _lightButton = [[UIButton alloc]initWithFrame:CGRectMake(0, (rect.origin.y + rect.size.height+40), kWidth, 30)];
...
- (void)moveUpAndDownLine {
    ...
    [UIView animateWithDuration:LINE_SCAN_TIME - 0.05
                     animations:^{
                         ...
                         (kHeight + viewFinderSize.height)/2 -
